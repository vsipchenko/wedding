...
lint-py:
	...
	# Requires flake8
	$(WORKON_HOME)/wedding/bin/flake8 .
...
setup:
	virtualenv -p `which python3.5` $(WORKON_HOME)/wedding
	$(WORKON_HOME)/wedding/bin/pip install -U pip wheel
	$(WORKON_HOME)/wedding/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/wedding/bin/pip freeze
	npm install
	npm update
	cp wedding/settings/local.example.py wedding/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=wedding.settings.local" > .env
	createdb -E UTF-8 wedding
	$(WORKON_HOME)/wedding/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The wedding project is now setup on your machine."
...
update:
	$(WORKON_HOME)/wedding/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
